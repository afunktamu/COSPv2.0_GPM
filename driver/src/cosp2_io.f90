...
!       if (status .ne. nf90_NoERR) print*,trim(nf90_strerror(status)) 
  ...
  SUBROUTINE NC_READ_INPUT_FILE(fname,Npnts,Nl,Nhydro,lon,lat,p,ph,z,zh,T,qv,rh,tca,cca, &
                                ...
                                fl_lsgrpl,fl_ccrain,fl_ccsnow,nc_lsliq,nc_lsice,         &
                                nc_lsrain,nc_lssnow,Reff,dtau_s,dtau_c,dem_s,            &
    ...
    real(wp),dimension(Npnts,Nl),target,intent(out) :: p,ph,z,zh,T,qv,rh,tca,cca, &
         ...
         fl_ccrain,fl_ccsnow,dtau_s,dtau_c,dem_s,dem_c,mr_ozone,nc_lsliq, &
         nc_lsice,nc_lsrain,nc_lssnow
    ...
    do vid = 1,nvars
       ...
       case ('nc_lsliq')
          if (Lpoint) then
             nc_lsliq(1:Npoints,:) = x2(1:Npoints,1:Nlevels)
          else
             call map_ll_to_point(Na,Nb,Npoints,x3=x3,y2=nc_lsliq)
          endif
       case ('nc_lsice')
          if (Lpoint) then
             nc_lsice(1:Npoints,:) = x2(1:Npoints,1:Nlevels)
          else
             call map_ll_to_point(Na,Nb,Npoints,x3=x3,y2=nc_lsice)
          endif
       case ('nc_lsrain')
          if (Lpoint) then
             nc_lsrain(1:Npoints,:) = x2(1:Npoints,1:Nlevels)
          else
             call map_ll_to_point(Na,Nb,Npoints,x3=x3,y2=nc_lsrain)
          endif
       case ('nc_lssnow')
          if (Lpoint) then
             nc_lssnow(1:Npoints,:) = x2(1:Npoints,1:Nlevels)
          else
             call map_ll_to_point(Na,Nb,Npoints,x3=x3,y2=nc_lssnow)
          endif
       case ('dtau_s')
          if (Lpoint) then
